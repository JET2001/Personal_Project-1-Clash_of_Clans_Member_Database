...
if (isset($_POST['submitdate']))
{
  ...
  if (count($rows) == 0)
  {
    ...
    header("Location: addmultiplecw.php");
    return;
  }
  else
  {
    ...
    header("Location: addmultiplecw.php?DateID=".$dateID."");
    return;
  }
}
...
if(isset($_POST['submit']))
{
  ...
  for($i = 1; $i <= 50; $i++)
  {
    ...
    else
    {
      ...
      $stmt = $pdo->prepare("INSERT INTO ClanWars (MemberID, Count, DateID) VALUES
      (:MemID, :co, :DateID)");
      $stmt->execute(array(':MemID'=>$MemID, ':co'=>$co, ':DateID'=>$mth));
    }
  }//endfor
...
<body class = "starter-template">
  <h1> Add Multiple Clan War Entries </h1>
  <p> <em> For either wrong entry troubleshoot or CWL Use </em></p>
  <?php errorMsgs() ?>
  <form method = "POST">
  ...
  {//After user sets date, then load the donation form.
    echo('
    <form method = "POST">
    <label for = "add_entry" name = "addmcw"> Click the "+" to add an entry: </label>
    <input type = "submit" name = "addmcw" id = "addmcw" value = "+"></input><br>
    <p><em> Avoid wrong entries by using the autocomplete function when keying in the member name.</em></p>
    <div id = "mcw_fields"></div>
  ...
  $(document).ready(function()
  {
    ...
    var source = $("#mcw_template").html(); //local variable source
    $("#mcw_fields").append(source.replace(/@COUNT@/g,countMems));
